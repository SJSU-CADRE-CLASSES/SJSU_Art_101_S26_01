...
float range(float vm, float vx, float value) {
  return (value - vm) / (vx - vm);
}
...
void drawHit(inout vec4 col, vec2 p, vec2 hitPos, float hitDist) {
    ...
    // Spread math modifies the falloff of the hit distances
...
void drawReflectedHit(inout vec4 col, vec2 p, vec2 hitPos, float hitDist, vec2 screenSize) {
    ...
    drawHit(col, p, ref(hitPos, vec2(0.0, 1.0), 1.0), hitDist);
    drawHit(col, p, ref(hitPos, vec2(0.0, -1.0), 1.0), hitDist);
    drawHit(col, p, ref(hitPos, vec2(1.0, 0.0), screenSize.x/screenSize.y), hitDist);
    drawHit(col, p, ref(hitPos, vec2(-1.0, 0.0), screenSize.x/screenSize.y), hitDist);
}
...
// Pseudo-random noise for dithering (replaces iChannel0)
float hash12(vec2 p) {
...
void main() {
    ...
    // We can confidently increase this to a high number because we will break out of the loop
    // the absolute moment the theoretical ripple fully fades down to 0% opacity.
    const int reflectionLimit = 15;

    // We collect the DVD mask as a separate minimum distance accumulator
    float finalDvdMask = 1.0;

    for(int idx = 0; idx < 5; ++idx) {
        if(idx >= uNumLogos) break;
        
        // Artificial offset parameters based on instance index to ensure totally unique tracks
        ...
        // Remap to zero-bottom-left bounded space
        move = move / size + 0.5;
        ...
        for (int i = 0; i < reflectionLimit; i++) {
            ...
            if (i > 0) {
                hitPos = calcHitPos(hitPos - 0.00001/size, activeDir, size);
            }
            lastHitPos = hitPos;
            
            float hitDist = distance(hitPos, move);
            
            // The visual ripple 'blend' opacity evaluates to absolutely 0.0 when hitDist reaches 3.0.
            // By breaking early here, we skip heavy calculations when ripples are invisible, 
            // vastly enhancing performance while completely preventing early ripple clipping!
        ...
        // Re-calculate move for the logo drawing offset
        flip(move);
        move = (move - 0.5) * size;
        
        // Update DVD distance mask additively for multi-logo
        float dMask = dvd((p - move) / logoScale);
        float fw = fwidth(dMask);
        if (fw <= 0.0) fw = 0.001; // safety against division by zero!
        dMask /= fw;
        dMask = 1.0 - clamp(dMask, 0.0, 1.0);
        finalDvdMask = min(finalDvdMask, 1.0 - dMask);
    }
        
    // Calculate normals from hit accumulations
    float bf = uBumpFactor; 
    ...
    lin += 0.2;
    col.rgb = col.rgb * lin;
    col.rgb += uSpecularIntensity * spe;
    
    // Draw the accumulated multi-logo masks
    col.rgb = mix(col.rgb, vec3(1.0), 1.0 - finalDvdMask);
    
    // Banding be gone (Replacing iChannel0 texture completely with hash procedural dither)
