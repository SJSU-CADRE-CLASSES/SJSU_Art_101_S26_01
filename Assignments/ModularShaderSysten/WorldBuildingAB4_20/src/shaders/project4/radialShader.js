export const vertexShader4Radial = `
varying vec2 vUv;
void main() {
    vUv = uv;
...
export const fragmentShader4Radial = `
...
uniform float uChromatic; // 0.0 for none, > 0.0 for effect
...
vec3 lOff() {    
    // The user decided to keep the swinging motion for peak dynamic effect!
...
vec4 texCA(sampler2D tex, vec2 uv, float ca) {
    return vec4(
        texture2D(tex, uv + vec2(ca, 0.0)).r,
        texture2D(tex, uv).g,
        texture2D(tex, uv - vec2(ca, 0.0)).b,
        texture2D(tex, uv).a
    );
}
...
void main() {
    ...
    vec4 col = texCA(tDiffuse, uv, uChromatic) * 0.25;
    ...
    for(float i=0.0; i < SAMPLES; i++) {
        uv -= dTuv;
        col += texCA(tDiffuse, uv, uChromatic) * currentWeight;
