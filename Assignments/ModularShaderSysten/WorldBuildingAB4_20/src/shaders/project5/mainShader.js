...
float constellations(vec3 rd) {
    ...
    for(float y = -1.0; y <= 1.0; y++) {
        for(float x = -1.0; x <= 1.0; x++) {
            vec2 cellOff = vec2(x, y);
            vec2 cellId = id + cellOff;
            
            vec2 n = hash32(vec3(cellId, jTime * uConstellationSpeed * 0.01));
            vec2 pt = cellOff + sin(n * 6.283) * uNoiseControl;
            p[i++] = pt;
        }
    }
    
    for(int j = 0; j < 9; j++) {
        float dPoint = length(gv - p[j]);
        m += smoothstep(0.04, 0.01, dPoint) * 0.8;
...
vec3 gsky(vec3 rd, vec3 ld, bool mask){
    ...
    vec3 back = vec3(0.4, 0.1, 0.7) * (1.0 - 0.5 * exp2(-0.1 * abs(length(rd.xz)/rd.y)) * max(sign(rd.y), 0.0));
...
void main() {
    ...
    if (d > 0.0) {
        ...
        vec3 gridColorDark = vec3(0.8, 0.1, 0.92);
        vec3 gridColorVapor = vec3(0.4, 0.5, 1.0);
        vec3 currentGridColor = mix(gridColorDark, gridColorVapor, uVaporwaveMix);
        
        col = mix(col, currentGridColor, smoothstep(uGridGlow, 0.0, i.y));
        col = mix(sky, col, fog);
