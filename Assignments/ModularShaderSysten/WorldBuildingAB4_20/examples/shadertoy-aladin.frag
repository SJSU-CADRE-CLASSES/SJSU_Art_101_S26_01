// Merged for this repo: legacy/shadertoy/star.frag (common) + image pass + WebGL entry.
// Upload this single file as the fragment shader. Pair with shadertoy-aladin.controls.json for sliders.
...
uniform float uTetraWireRadius;
uniform float uRefractionIndex;
uniform float uIorAir;
uniform float uGlowBase;
uniform float uGlowColorK;
uniform float uSurfTint;
uniform float uClipRadius;
uniform float uGlitterFreq;
uniform float uGlitterRare;
uniform float uGlitterInt;
uniform float uIncThick;
uniform float uRays;
uniform float uRaysSmooth;
uniform float uEnableGlitter;
uniform float uHitStep;
uniform float uHitPush;
uniform float uMarchK;

// ----- begin star.frag (common) -----
...
#define MERKABA_WIRES 0
#define PALETTE_IDLE 6
#define RayMarchSteps 112
#define T (iTime*uSpeed/6.)
#define col C.rgb
#define res iResolution.xy
#define N(p) ((1./uZoom)*((p)-res*0.5)/min(res.y,res.x))
...
mat3 axisAngleMat(vec3 v, float a)
{
    mat3 m;
    vec3 w;
    vec3 b1;
...
float glitterField(vec3 pb, vec3 rd)
{
    float n = noise3d(pb * ( uGlitterFreq*iResolution.y/550. ) + 2.3);
    float b = smoothstep( uGlitterRare, 1.0, n);
    float rim = pow(clamp(1.0 - abs(dot(normalize(pb), normalize(rd))), 0.0, 1.0), 3.0);
    return b * (0.25 + 0.75*rim);
}

vec3 toBase(vec3 p, mat2 R)
{
    vec3 b = p;
    b.xy = R*b.xy;
    b.yz = R*b.yz;
...
vec2 mapBase(vec3 p, mat2 tetTilt, mat3 tetSpin)
{
    vec3 q = p;
    float dMin = 1e6;
    float iMin = 0.0;
    ...
    q.yz = tetTilt * q.yz;
    for (int i = 0; i < 5; ++i) {
        float d = tetFrame(q, uTetraWireRadius);
#if ( MERKABA_WIRES == 1 )
              d = min( tetFrame( -q, uTetraWireRadius ), d );
#endif
        if (d < dMin) {
            dMin = d;
            iMin = float(i);
    ...
    dMin = max(dMin, sphere(p, uClipRadius));
    return vec2(dMin, iMin);
}

vec3 normal(vec3 p, mat2 R, mat2 tetTilt, mat3 tetSpin)
{
...
vec3 shadeFast(vec3 surfCol, vec3 Nn, vec3 rd, float insideMask)
{
    vec3 L = getLightDir();
    vec3 V = normalize(-rd);
...
void mainImage(out vec4 C, in vec2 POS)
{
    ...
    for(int i=0;i<RayMarchSteps;i++)
    {
        ...
        col += uGlowBase/(dd*dd) * surfCol*glowCol * glow;

        if ( inside ) { col += surfCol/50.0; }
        ...
        if (!inside && (uRays > 0.5) )
        {
            ...
            if ( (uRaysSmooth > 0.5) )
            {
                if( (i & 3) == 0 ) { col = ( col + tonemap2( col ) )/1.75; }
            }
        }

        if (uEnableGlitter > 0.5) {
            float matMask = smoothstep(uIncThick, 0.0, ad);
            if(matMask > 0.001)
            {
                float bub = glitterField(baseP, rd);
                col += uGlitterInt * bub * matMask * (0.35 + 0.85*surfCol);
            }
        }
        ...
        if(ad < HIT_EPS)
        {
            ...
            float etaI = curIOR;
            float etaT = entering ? uRefractionIndex : uIorAir;
            ...
            else
            {
                rd = refr;
                inside = entering;
                curIOR = inside ? uRefractionIndex : uIorAir;
            }
