...
// Raymarched organic structure with procedural sine noise.
...
float map(vec3 p) {
    ...
    p = rotateX(p, iMouse.y / iResolution.y * 3.14);
    p = rotateY(p, iMouse.x / iResolution.x * 3.14);
...
void main() {
    ...
    for (int r = 1; r <= rayCount; r++) {
        ...
        if (d < 0.01 || r == rayCount) {
            ...
            float nearFade = smoothstep(0.1, 1.2, t);
