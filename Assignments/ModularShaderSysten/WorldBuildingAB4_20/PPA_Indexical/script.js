...
attractor.internalSubsteps = 500;
attractor.integratorFidelity = 1;
...
// ── 3D UI Primitives ─────────────────────────────────────────────────
const uiMaterial = new THREE.LineBasicMaterial({ color: 0x00aaff, transparent: true, opacity: 0.4 });
const uiGroup = new THREE.Group();

// Parent UI to the camera so controls are always in view regardless of orbit
scene.add(camera);
camera.add(uiGroup);

// Chaos Ring — centered in camera view; scrubs b (chaos level)
const ringMat  = uiMaterial.clone();
const ringMesh = new THREE.LineSegments(
    new THREE.EdgesGeometry(new THREE.TorusGeometry(5.4, 0.21, 5, 9)),
    ringMat
);
ringMesh.position.set(0, 0, -8);
uiGroup.add(ringMesh);

// Speed Node — right edge of camera view; drag vertically to control dt
// top of screen = fastest, bottom = slowest
const chaosMat  = uiMaterial.clone();
const chaosMesh = new THREE.LineSegments(
    new THREE.EdgesGeometry(new THREE.IcosahedronGeometry(0.45, 0)),
    chaosMat
);
// y range [-3, 3] → dt [0.00001, 3.5]  (starts at centre = mid-range speed)
chaosMesh.position.set(4.5, 0, -8);
uiGroup.add(chaosMesh);
...
let isDraggingChaosNode  = false;
let ringNormPos          = 0;   // [0, 1] — scrub position across full b range
let prevMouseClientX     = 0;
let prevMouseClientY     = 0;

function updateMouseVec(e) {
    mouse.x =  (e.clientX / window.innerWidth)  * 2 - 1;
    mouse.y = -(e.clientY / window.innerHeight) * 2 + 1;
}

window.addEventListener('pointerdown', (e) => {
    // Only raycast if clicking directly on the canvas (ignore UI clicks)
    if (e.target !== renderer.domElement) return;

    updateMouseVec(e);
    raycaster.setFromCamera(mouse, camera);
    if (raycaster.intersectObject(ringMesh,    false).length) isDraggingRing      = true;
    if (raycaster.intersectObject(chaosMesh,   false).length) isDraggingChaosNode = true;
    prevMouseClientX = e.clientX;
    prevMouseClientY = e.clientY;
});

window.addEventListener('pointermove', (e) => {
    if (!isDraggingRing && !isDraggingChaosNode) return;
    const dx = e.clientX - prevMouseClientX;
    const dy = e.clientY - prevMouseClientY;
    prevMouseClientX = e.clientX;
    prevMouseClientY = e.clientY;

    if (isDraggingRing) {
        ringNormPos = Math.max(0, Math.min(1, ringNormPos + dx * 0.005));
        ringMesh.rotation.z = ringNormPos * Math.PI * 2;
        const newB = 0.00001 + ringNormPos * (0.33 - 0.00001);
        attractor.b = newB;
        lfoParamConfigs.b.base        = newB;
        lfoParamConfigs.b.slider.value = String(newB);
        valB.textContent               = newB.toFixed(5);
        setSynthParam('b', newB, 0.01);
    }

    if (isDraggingChaosNode) {
        // Camera-local y range: [-3, 3] → dt [0.00001, 3.5]
        // dy is negative when dragging up (screen Y decreases = faster)
        const newY = Math.max(-3, Math.min(3, chaosMesh.position.y - dy * 0.012));
        chaosMesh.position.y = newY;
        const newDt = 0.00001 + ((newY + 3) / 6) * (3.5 - 0.00001);
        attractor.dt = newDt;
        lfoParamConfigs.dt.base        = newDt;
        lfoParamConfigs.dt.slider.value = String(newDt);
        lfoParamConfigs.fineDt.base     = Math.min(0.03, newDt);
        sliderFineDt.value              = String(Math.min(0.03, newDt));
        valDt.textContent               = newDt.toFixed(5);
        setSynthParam('dt', newDt, 0.01);
    }
});

window.addEventListener('pointerup', () => {
    isDraggingRing      = false;
    isDraggingChaosNode = false;
});
...
const lfoParamConfigs = {
    ...
    orbitRadius: { label: 'Camera Orbit Radius', slider: sliderCamZ, min: 1, max: 20, step: 0.1, base: 12.4, set: (v) => { cameraOrbitAmplitude = v; valCamZ.textContent = v.toFixed(1); } },
    ...
    trailLength: { label: 'Trail Length', slider: sliderTrailLength, min: 0, max: maxPoints, step: 1, base: 550, set: (v) => { currentTrailLength = Math.max(0, Math.min(maxPoints, Math.round(v))); valTrailLength.textContent = String(currentTrailLength); } },
    ...
    internalSubsteps: { label: 'Internal Substeps', slider: sliderInternalSubsteps, min: 1, max: 500, step: 1, base: 500, set: (v) => { attractor.internalSubsteps = Math.max(1, Math.round(v)); valInternalSubsteps.textContent = String(attractor.internalSubsteps); } },
    integratorFidelity: { label: 'Integrator Fidelity', slider: sliderIntegratorFidelity, min: 0, max: 1, step: 0.01, base: 1, set: (v) => { attractor.integratorFidelity = v; valIntegratorFidelity.textContent = v.toFixed(2); } },
...
sliderTrailLength.value = '550';
valTrailLength.textContent = '550';
...
toggleAudioX.addEventListener('change', (e) => {
    ...
    setVoiceEnableParams();
    if (hudFmX) hudFmX.checked = audioXEnabled;
});
...
togglePmX.addEventListener('change', (e) => {
    ...
    setVoiceEnableParams();
    if (hudPmX) hudPmX.checked = audioPmXEnabled;
});
...
const syncChannel = new BroadcastChannel('thomas-sync');
...
function loop() {
    ...
    // ── 3D UI physics & animation ────────────────────────────────────
    // Chaos Ring: mirror attractor.b unless actively dragging
    if (!isDraggingRing) {
        ringNormPos = Math.max(0, Math.min(1, (attractor.b - 0.00001) / (0.33 - 0.00001)));
        ringMesh.rotation.z = ringNormPos * Math.PI * 2;
    }

    // Chaos Node: mirror attractor.dt unless actively dragging
    if (!isDraggingChaosNode) {
        chaosMesh.position.y = Math.max(-3, Math.min(3, ((attractor.dt - 0.00001) / (3.5 - 0.00001)) * 6 - 3));
    }
    
    // Slow ambient rotation for visual presence
    chaosMesh.rotation.y += 0.008;
    chaosMesh.rotation.x += 0.004;
    ...
    // Update Telemetry Display — fixed-width format prevents jitter
    const tfmt = (v) => (v >= 0 ? '+' : '') + v.toFixed(3);
    const telemetryText = `X: ${tfmt(displayX)}   Y: ${tfmt(displayY)}   Z: ${tfmt(displayZ)}`;
    telemetryDisplay.textContent = telemetryText;
    if (hudTelemetry) hudTelemetry.textContent = telemetryText;

    // Flag geometry update
    trail.geometry.attributes.position.needsUpdate = true;
    trail.geometry.setDrawRange(maxPoints - currentTrailLength, currentTrailLength);

    // Synchronize Projector Mode
    if (window.syncChannel) {
        window.syncChannel.postMessage({
            ...
            positions: positions,
            uiVisible: uiGroup.visible,
            ringRotZ: ringMesh.rotation.z,
            chaosPosY: chaosMesh.position.y,
            chaosRotX: chaosMesh.rotation.x,
            chaosRotY: chaosMesh.rotation.y
        });
    }
...
// ── HUD Sound Engine controls ────────────────────────────────────────
const hudFmX     = document.getElementById('hud-fm-x');
const hudPmX     = document.getElementById('hud-pm-x');
const hudGain    = document.getElementById('hud-gain');
const hudGainVal = document.getElementById('hud-gain-val');
const hudResetBtn = document.getElementById('hud-reset');
const hudTelemetry = document.getElementById('hud-telemetry');

hudFmX.addEventListener('change', (e) => {
    resumeAudioCtx();
    if (!synthNode) setupChaosSynth();
    audioXEnabled = !!e.target.checked;
    // Keep the hidden panel toggle in sync
    if (toggleAudioX) {
        toggleAudioX.checked = audioXEnabled;
    }
    setVoiceEnableParams();
});

hudPmX.addEventListener('change', (e) => {
    resumeAudioCtx();
    if (!synthNode) setupChaosSynth();
    audioPmXEnabled = !!e.target.checked;
    // Keep the hidden panel toggle in sync
    if (togglePmX) {
        togglePmX.checked = audioPmXEnabled;
    }
    setVoiceEnableParams();
});

hudGain.addEventListener('input', (e) => {
    resumeAudioCtx();
    const val = parseFloat(e.target.value);
    masterGainNode.gain.setTargetAtTime(val, audioCtx.currentTime, 0.02);
    hudGainVal.textContent = val.toFixed(2);
    // Keep the hidden master volume slider in sync
    const panelVol = document.getElementById('slider-master-volume');
    if (panelVol) panelVol.value = String(val);
    const panelVolVal = document.getElementById('val-master-volume');
    if (panelVolVal) panelVolVal.textContent = val.toFixed(2);
});

hudResetBtn.addEventListener('click', () => {
    resumeAudioCtx();
    attractor.reset();
    if (synthNode) synthNode.port.postMessage({ type: 'reset' });
});

// ── Advanced Controls toggle ─────────────────────────────────────────
const hudDefaultUi = document.getElementById('hud-default-ui');
document.getElementById('btn-advanced-controls').addEventListener('click', () => {
    [leftPanelStack, rightPanelStack, soundEnginePanel, hudDefaultUi].forEach(el => el.classList.toggle('ui-hidden'));
    uiGroup.visible = !uiGroup.visible;  // hide 3D controls when HTML panels are shown
});

const btnProjector = document.getElementById('btn-projector');
if (btnProjector) {
    btnProjector.addEventListener('click', () => {
        window.open('projector.html', 'ProjectorMode', 'width=1280,height=720,menubar=no,toolbar=no,location=no,status=no');
    });
}
